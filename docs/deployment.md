...
3. Copy `deploy/nginx/*.conf` to `/etc/nginx/sites-available/<domain>` (drop the `.conf` extension — this host's other sites and certbot both key off the extensionless filename, and `sites-enabled/<domain>` symlinks to that; a `<domain>.conf` copy left alongside it is inert — nginx never reads it, so editing/rsyncing it silently does nothing — this bit us once when adding the customer-bot webhook location), symlink into `sites-enabled/`, `nginx -t && systemctl reload nginx`.
4. Once DNS resolves: `certbot --nginx -d mydoners.uz -d www.mydoners.uz`, `certbot --nginx -d api.mydoners.uz`, `certbot --nginx -d admin.mydoners.uz` (matches this host's existing certbot-per-domain pattern). Certbot edits the extensionless `sites-available/<domain>` file in place, adding the actual `listen 443 ssl` block and cert paths — from here on, **that file is the only one that matters**; the repo's `deploy/nginx/*.conf` is just the seed template.
5. Copy `.env.prod.example` → `.env.prod`, `backend/.env.prod.example` → `backend/.env.prod`, `courier-bot/.env.prod.example` → `courier-bot/.env.prod`, `customer-bot/.env.prod.example` → `customer-bot/.env.prod`, fill in real secrets (bot tokens, generated `JWT_SECRET`/`COURIER_BOT_API_KEY`/`ADMIN_PASSWORD` via `openssl rand -hex 32` or a memorable password for `ADMIN_PASSWORD` specifically since a human types that one, matching Postgres password in both `.env.prod` and `backend/.env.prod`'s `DATABASE_URL`). `customer-bot/.env.prod`'s `TELEGRAM_BOT_TOKEN` must exactly match `backend/.env.prod`'s — it's the same bot (@mydoner_bot) the Mini App's initData is signed against.
...
11. Bot setup: `setChatMenuButton` on the customer bot pointing to `https://mydoners.uz` (Telegram Bot API, one-time call); the courier bot and customer bot both register their own webhooks automatically on container start (`BOT_MODE=webhook` calls `setWebhook` itself — see courier-bot/src/index.ts and customer-bot/src/index.ts). The customer bot (`@mydoner_bot`, port 4212, path `/customer-bot/webhook`) handles `/start` and `/help` outside the Mini App — it does not touch ordering.
